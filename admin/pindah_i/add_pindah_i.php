...
if (isset($_POST['simpan'])) {
	...
	$sql_check_data = mysqli_query($koneksi, "SELECT * FROM tb_pindah_bjm_i WHERE nopol='$nopol'") or die(mysqli_error($koneksi));
	...
	if (mysqli_num_rows($sql_check_data) > 0) {
		...
		$query = mysqli_query($link, "INSERT INTO tb_pindah_bjm_i(id_petugas_bbnkb, nopol, nama_stnk, alamat_lama, alamat_baru, tgl) VALUES('$id_petugas_bbnkb','$nopol','$nama_stnk','$alamat_lama','$alamat_baru','$tgl')");
		if ($query) {
			echo "<script>
			...
			Swal.fire({title: 'Tambah Data Gagal',text: '',icon: 'error',confirmButtonText: 'OK'
				...
				window.location = 'index.php?page=add-pindah_i';
				}
			})</script>";
		}
	}
